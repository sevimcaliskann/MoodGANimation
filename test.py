...
class MorphFacesInTheWild:
    ...
    def save_as_gif(self, morphed_video, name):
        output_name = os.path.join(self._out_dir, \
                '{0}_epoch_{1}_out.gif'.format(name, \
                str(self._opt.load_epoch)))
        imageio.mimsave(output_name, list(morphed_video))
        print('Morphed video is saved at path {}'.format(output_name))

    def tile_frames(self, morphed_video, name):
        output_name = os.path.join(self._out_dir, \
                '{0}_epoch_{1}_out.png'.format(name, \
                str(self._opt.load_epoch)))
        tiled_img = np.concatenate(morphed_video, axis=1)
        self._save_img(tiled_img, output_name)
        print('Tiled image is saved at path {}'.format(output_name))
    ...
    def random_generation(self, get_start_from_video=False):
        ...
        expression = np.concatenate((val,aro), axis=1) if self._opt.cond_nc == 2 else \
                        np.concatenate((val,third, third, fourth), axis=1)

        if not get_start_from_video:
            morphed_video = self.morph_file(self._opt.input_path, expression)
        else:
            img, _ = self.get_n_faces_from_video(1)
            morphed_video = self.morph_file(self._opt.groundtruth_video, expression, img=img)

        name = os.path.basename(self._opt.groundtruth_video)[:-4] if get_start_from_video else \
               os.path.basename(self._opt.input_path)[:-4]
        self.tile_frames(morphed_video['fake_imgs_masked'], name + '_fake_imgs_masked')
        self.tile_frames(morphed_video['fake_imgs'], name + '_fake_imgs')
        self.tile_frames(morphed_video['img_mask'], name + '_img_mask')
    ...
    def generate_from_groundtruth(self):
        ...
        name = os.path.basename(self._opt.groundtruth_video)[:-4]
        self.tile_frames(ground_faces, name + '_groundtruth')
        self.save_as_gif(ground_faces, name + '_groundtruth')

        start_face = ground_faces[0] if self._opt.frames_cnt>1 else np.copy(ground_faces)

        anns = np.concatenate(anns, axis=0)
        morphed_video = self.morph_file(self._opt.groundtruth_video, anns, img=start_face)
        self.tile_frames(morphed_video['fake_imgs_masked'], name + '_fake_imgs_masked')
        self.tile_frames(morphed_video['fake_imgs'], name + '_fake_imgs')
        self.tile_frames(morphed_video['img_mask'], name + '_img_mask')
...
def main():
    ...
    morph.random_generation(get_start_from_video=False)
    _, expression = morph.generate_from_groundtruth()
    img = cv_utils.read_cv2_img(opt.input_path)
    morphed_video = morph.morph_file(opt.input_path, expression, img=img)
    morph.save_as_gif(morphed_video['concat'], os.path.basename(opt.input_path)[:-4])
