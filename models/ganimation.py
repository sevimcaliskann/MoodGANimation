...
class GANimation(BaseModel):
    ...
    def _create_temp_discriminator(self):
        return NetworksFactory.get_by_name('discriminator_temporal', c_dim=self._opt.cond_nc, image_size=self._opt.image_size)
    ...
    def _init_losses(self):
        ...
        self._loss_g_bidirec_cyc = Variable(self._Tensor([0]))
        ...
        self._loss_g_frame_cond = Variable(self._Tensor([0]))
        ...
        self._loss_d_frame_cond = Variable(self._Tensor([0]))
    ...
    def _bidirectional_loss(self, fake_videos_masked):
        loss = 0
        ...
        for idx in range(self._opt.frames_cnt-3, -1, -1):
            ...
            fake_img_mask = self._do_if_necessary_saturate_mask(fake_img_mask, saturate=self._opt.do_saturate_mask)
            fake_imgs_masked = fake_img_mask * last + (1 - fake_img_mask) * fake_imgs
            loss += self._criterion_cycle(cyc_img, fake_imgs_masked)*self._opt.lambda_bidirec_cyc

        real_cond = self._annotations[:, 0, :]
        fake_imgs, fake_img_mask = self._G.forward(last, real_cond)
        fake_img_mask = self._do_if_necessary_saturate_mask(fake_img_mask, saturate=self._opt.do_saturate_mask)
        fake_imgs_masked = fake_img_mask * last + (1 - fake_img_mask) * fake_imgs
        loss += self._criterion_cycle(self._first, fake_imgs_masked)*self._opt.lambda_bidirec_cyc
        return loss
    ...
    def _forward_G(self, keep_data_for_visuals):
        ...
        for idx in range(1, self._opt.frames_cnt):
            ...
            d_fake_desired_img_masked_prob, d_fake_frame_cond, feats_disc = self._D.forward(fake_imgs_masked, feats_disc)
    ...
    def _forward_D(self):
        ...
        self._loss_d_frame_cond = 0
        feats_fake = None
        feats_disc_fake = None
        feats_disc_real = None
        fake_videos_masked = list()
        for idx in range(1, self._opt.frames_cnt):
            real_img = self._frames[:, idx, :, :, :]
            real_cond = self._annotations[:, idx, :]
        ...
        return self._loss_d_real + self._loss_d_fake + self._loss_d_frame_cond, \
    ...
    def get_current_errors(self):
        loss_dict = OrderedDict([#('g_fake', self._loss_g_fake.detach()),
                                 ...
                                 ('g_frame_cond', self._loss_g_frame_cond.detach()),
                                 ...
                                 ('d_frame_cond', self._loss_d_frame_cond.detach()),
    ...
    def load(self):
        ...
        if self._is_train:
            ...
            #self._D_temp.load_from_checkpoint(self._save_dir, load_epoch)
